...
async function seed() {
	try {
		...
		const ultimateTeam = await Team.create({
			name: 'Sacha',
			description: 'Un jour je serais le meilleur dresseur. Je me battrais sans répits !',
		...
		await mewtwo.addVoter(profile);
		await mewtwo.addVoter(profile2);
		await mew.addVoter(profile);
		await dracolosse.addVoter(profile2);
		await staross.addVoter(profile);
		await staross.addVoter(profile2);
		await ronflex.addVoter(profile);
		await poissoroy.addVoter(profile);
		await soporifik.addVoter(profile);
		await kokiyas.addVoter(profile);
		await magneti.addVoter(profile2);
	} catch (error) {
